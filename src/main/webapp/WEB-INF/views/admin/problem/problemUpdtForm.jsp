			...
			<div class="col-sm-9">
				...
				<c:if test="${problemDetail.type == '1' }">
					<form name="ox_writeForm" id="ox_writeForm" method="post">
						<input type="hidden" name="problem_seq"
							value="${problemDetail.problem_seq}" /> <input type="hidden"
							name="type" value="1" />
						<div class="container">
							...
							<table class="table table-hover">
								...
								<tr>
									...
									<td><textarea id="problem" name="problem"
											class="form-control" rows="20" cols="200"> ${problemDetail.problem} </textarea></td>
								</tr>

								<tr>
									<td width="7%">카테고리</td>
									<td><select name="category_seq" id="category_seq"
										class="form-control" style="width: 200px; height: 40px;">
											<option value="">카테고리 선택</option>
											<c:forEach items="${categoryList}" var="categoryList"
												varStatus="status">
												<option value="${categoryList.category_seq}"
													<c:if test="${problemDetail.category_seq == categoryList.category_seq}">selected</c:if>>${categoryList.category_name}</option>
											</c:forEach>
									</select></td>
								</tr>
								...
								<tr>
									...
									<td><c:forEach items="${optionDetail}" var="optionDetail"
											varStatus="status">
											<input type="radio" value="O" name="option_contents"
												<c:if test="${optionDetail.option_contents == 'O' }">checked</c:if> />O
					        		<input type="radio" value="X" name="option_contents"
												<c:if test="${optionDetail.option_contents == 'X' }">checked</c:if> />X
					        	</c:forEach></td>
								</tr>
							</table>
							...
							<table class="table table-hover">
								<tr>
									<td><input type="button" value="취소" onclick="goList();"
										...
										type="button" value="수정" onclick="goUpdt('ox'); return false;"
				...
				<c:if test="${problemDetail.type == '2' }">
					<form name="ch_writeForm" id="ch_writeForm" method="post">
						<input type="hidden" name="problem_seq"
							value="${problemDetail.problem_seq}" /> <input type="hidden"
							name="answer_no" id="answer_no" /> <input type="hidden"
							name="type" value="2" />
						<div class="container">
								...
								<c:forEach items="${optionDetail}" var="optionDetail"
									...
									<tr>
										...
										<td><input type="text" class="form-control"
											id="option_contents" name="option_contents"
											value="${optionDetail.option_contents}" /></td>
										...
										<td><input type="radio" id="answer_seq" name="answer_seq"
											...
											<c:if test="${optionDetail.answer_yn == 'Y'}">checked</c:if>></td>
									</tr>
								</c:forEach>
							</table>
							...
							<table class="table table-hover">
								<tr>
									<td><input type="button" value="취소" onclick="goList();"
										class="btn btn-default"
										style="float: right; margin-right: 5px" /> <input
										type="button" value="수정"
										onclick="goUpdt('choice'); return false;"
										class="btn btn-default"
										style="float: right; margin-right: 5px" /></td>
								</tr>
							</table>
